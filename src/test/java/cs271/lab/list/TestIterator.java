...
public class TestIterator {
  ...
  public void testRemove() {
    ...
    final var f = list.iterator();
    assertTrue(f.hasNext());
    assertEquals(33, f.next().intValue());
    // TODO fix the expected values in the assertions below [COMPLETE]
    assertTrue(f.hasNext());
    assertEquals(44, f.next().intValue());
    assertTrue(f.hasNext());
    assertEquals(55, f.next().intValue());
    assertTrue(f.hasNext());
    assertEquals(66, f.next().intValue());
  ...
  public void testAverageValues() {
    ...
    double sum = 0;
    int n = 0;
    // TODO use an iterator and a while loop to compute the average (mean) of the values [COMPLETE]
